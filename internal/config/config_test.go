...
// configEnvVars enumerates every environment variable consumed by
// Config.Load() per the envconfig tags on Config and its sub-structs.
// Kept in sync manually with the struct tags; if you add an envconfig
...
// clearConfigEnv removes every Config env var for the duration of t
// and restores the original values on cleanup. Use in tests that
// assert specific post-Load field values; without it an inherited env
// var (e.g. SHELLEY_TRANS_EPOCH from CI) writes into c before YAML
// runs and silently invalidates the assertions.
...
// TestLoadWithFlagsCLIOverYAML verifies the documented precedence
// CLI > YAML > env. LoadWithFlags snapshots Changed flags before
// env/yaml load and re-applies via fs.Set, so the pflag-bound CLI
// value survives yaml.Unmarshal writing through the same pointer.
...
// TestLoadWithFlagsUnsetFlagDefersToEnv proves flags the user did
// NOT pass (Changed=false) are NOT re-applied — otherwise defaults
// would clobber env/yaml. Here INPUT is only in env; CLI is empty;
// env must win.
...
// TestLoadDelegatesToLoadWithFlags pins that the no-flags Load()
// keeps the YAML > env > default behavior — cmd/adder-tray and
// tray/setup/store still call Load() without a FlagSet.
