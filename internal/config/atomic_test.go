...
func TestSaveAtomicSerializesPerPath(t *testing.T) {
	...
	for range n {
		wg.Go(func() {
			require.NoError(t, SaveAtomic(path, &spyPayload{
				inFlight:    &inFlight,
...
func TestSaveAtomicSerializesDifferentPathsConcurrent(t *testing.T) {
	...
	for i := range n {
		wg.Go(func() {
			...
			require.NoError(
				t,
				SaveAtomic(path, map[string]int{"i": i}),
			)
		})
	}
	wg.Wait()
...
func TestSaveAtomicCtx_ReturnsContextErrorIfAlreadyCancelled(t *testing.T) {
	path := filepath.Join(t.TempDir(), "out.yaml")
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	err := SaveAtomicCtx(ctx, path, map[string]int{"x": 1})
	require.Error(t, err)
	...
	assert.True(
		...
		"cancelled-up-front SaveAtomicCtx must not write",
	)
}

func TestSaveAtomicCtx_SuccessPath(t *testing.T) {
	path := filepath.Join(t.TempDir(), "out.yaml")
	err := SaveAtomicCtx(
		context.Background(),
		path,
	...
	buf, err := os.ReadFile(path)
	require.NoError(t, err)
	assert.Contains(t, string(buf), "x: 1")
	assertNoTmpResidue(t, path)
}

// TestSaveAtomicCtx_CancelDuringSlowMarshalReturnsEarly verifies that
// cancellation during a slow encode unblocks the caller. The
// underlying goroutine completes (Go can't interrupt syscalls) — we
// only assert the caller returned ctx.Err() promptly, not that the
// I/O was aborted.
func TestSaveAtomicCtx_CancelDuringSlowMarshalReturnsEarly(t *testing.T) {
	path := filepath.Join(t.TempDir(), "out.yaml")
	...
	start := time.Now()
	err := SaveAtomicCtx(ctx, path, blockingPayload{block: 5 * time.Second})
	elapsed := time.Since(start)

	require.Error(t, err)
	assert.ErrorIs(t, err, context.DeadlineExceeded)
	// The caller must return shortly after the context deadline, not
	// after the full 5-second marshal.
	assert.Less(
		t,
		elapsed,
		1*time.Second,
		"SaveAtomicCtx must return promptly on ctx cancel",
	)
}
...
func (p blockingPayload) MarshalYAML() (any, error) {
	time.Sleep(p.block)
	return map[string]int{"x": 1}, nil
}
