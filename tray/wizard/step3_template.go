...
// add validates v and appends it to the section on success. On failure
// the inline error is shown and the entry is left intact so the user
// can correct it.
...
func (s *templateStep) Apply(plan *setup.SetupPlan) {
	...
	if s.everythingCheck.Checked {
		...
		plan.Filter.Wallets = append([]string(nil), s.wallets.values...)
		plan.Filter.DReps = append([]string(nil), s.dreps.values...)
		plan.Filter.Pools = append([]string(nil), s.pools.values...)
	}
