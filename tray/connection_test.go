...
func TestConnectionManager_Reconnect_RemoteDrop(t *testing.T) {
	...
	// The EventClient should auto-reconnect after the drop. Poll the
	// server-side counter rather than asserting once — the client can
	// observe StatusConnected before the server handler executes the
	// counter increment, so a one-shot assertion races with it.
